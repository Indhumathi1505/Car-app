...
export default function Favourites() {
    ...
    const fetchFavourites = async () => {
      try {
        ...
        const favRes = await axios.get(
          `http://car-backend-final.onrender.com/api/favorites/${email}`
        );

        // 2️⃣ Fetch full car details
        const cars = await Promise.all(
          favRes.data.map(async (fav) => {
            const carUrl = `http://car-backend-final.onrender.com/api/cars/${fav.carId}`;
  ...
  const handleRemove = async (car) => {
    try {
      await axios.delete(
        `http://car-backend-final.onrender.com/api/favorites/${email}/${car.carId}`
      );
